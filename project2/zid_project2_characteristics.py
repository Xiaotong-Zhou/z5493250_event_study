...
def vol_cal(ret, cha_name, ret_freq_use: list):
    ...
    # Select the DataFrame for the specified frequency of data usage
    df = ret[ret_freq_use[0]].copy()  # Assuming ret_freq_use[0] is 'Daily' or 'Monthly'

    # Convert date index to a monthly PeriodIndex for monthly grouping
    df.index = pd.to_datetime(df.index).to_period('M')

    # Calculate the standard deviation each month, only if the data count for the month is 18 or more
    def calculate_volatility(group):
        if len(group) >= 18:
            return group.std(ddof=0)
        else:
            return None

    vol_df = df.groupby(df.index).apply(calculate_volatility)

    # Append the characteristic name to column names
    vol_df.columns = [f"{col}_{cha_name}" for col in vol_df.columns]
    # Replace 0.00000 values with NaN in the characteristics DataFrame
    vol_df.replace(0, float('NaN'), inplace=True)
...
def merge_tables(ret, df_cha, cha_name):
    ...
    # Extract DataFrame for monthly stock returns
    ret_monthly = ret['Monthly'].copy()

    # Replace 0.00000 values with NaN in the characteristics DataFrame
    df_cha.replace(0, float('NaN'), inplace=True)

    # Merge with the characteristics DataFrame and shift the characteristics columns one month forward
    merged_df = ret_monthly.merge(df_cha, how='left', left_index=True, right_index=True)
    merged_df[df_cha.columns] = merged_df[df_cha.columns].shift(1)
...
def cha_main(ret, cha_name, ret_freq_use: list):
    ...
    ret : dict
        A dictionary containing two items, where each item is a DataFrame that provides daily and monthly returns.
        See the docstring of the `aj_ret_dict` function in zid_etl.py for a description of this dictionary.
    ...
    # Merge the calculated characteristics with the monthly return table
